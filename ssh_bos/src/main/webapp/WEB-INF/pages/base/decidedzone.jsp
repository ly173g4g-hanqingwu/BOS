...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(e.length != 1){
			$.messager.alert("提示信息","请选中一个定区进行操作","warning");
		}else{
			$('#customerWindow').window('open');
			//清空下拉框里面的数据
			$('#noassociationSelect').empty();
			$('#associationSelect').empty();
			var url_1 = "decidedzoneAction_findCustomerNotAssociation.action";
			var decidedzoneid = e[0].id;
			$.post(url_1,function(data){
				for(var i = 0; i < data.length;i++){
					...
					name = name + "(" +telephone+")";
					$("#noassociationSelect").append("<option value = "+id+">"+name+"</opyion>");
					
				}
			});
			
			var url_2 = "decidedzoneAction_findCustomerAssociation.action";
			$.post(url_2,{"id":decidedzoneid},function(data){
				for(var i = 0; i < data.length;i++){
					...
					var telephone = data[i].telephone;
					name = name + "(" +telephone+")";
					$("#associationSelect").append("<option value = "+id+">"+name+"</opyion>");
	...
	function doDblClickRow(index,data){
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "decidedzoneAction_findCustomerAssociation?id="+data.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
